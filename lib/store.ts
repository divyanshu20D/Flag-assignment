...
// simple subscriber pattern to emulate "live" updates
type Listener = () => void
...
export async function updateFlag(key: string, patch: Partial<Omit<FeatureFlag, "key" | "updatedAt">>) {
  ...
  const next: FeatureFlag = {
    ...current,
    ...patch,
    // deep replace rules if provided
...
export async function listAuditLogs(opts?: {
  ...
  const filtered = opts?.flagKey ? auditLogs.filter((a) => a.flagKey === opts!.flagKey) : auditLogs
